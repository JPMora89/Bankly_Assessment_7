...
// Here we are correctly using the function "createToken" from the file. On our routes we use the
// function "createUserToken" from the file "auth.js" that is different from the function "createToken"
...
// Here the test correctly uses jwt.verify to verify the token created by the function "createToken".
// In the authUser function we use the function "jwt.decode" to verify the token created by the function
// which is incorrect. The function "jwt.decode" does not verify the token, it only decodes it.
...
// Test for GET /users/[username] showing bug in code, using "username, password" instead of "_token"
// Should return error that username is not defined
describe("GET /users", function() {
  test("should deny access if no token provided", async function() {
    const response = await request(app).get("/users");
    expect(response.statusCode).toBe(401);
...
// Test for GET /users/[username] showing bug in code, if user inputs wrong data there is no error thrown.
// No error is thrown, but the test fails because the user is not found.
describe("GET /users/[username]", function() {
  test("should deny access if no token provided", async function() {
    const response = await request(app).get("/users/u1");
    expect(response.statusCode).toBe(401);
